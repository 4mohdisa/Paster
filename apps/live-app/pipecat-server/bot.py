...
from pipecat.frames.frames import Frame, InputImageRawFrame, OutputImageRawFrame, LLMMessagesAppendFrame
...
async def run_bot(webrtc_connection):
    ...
    async def on_client_message(rtvi, msg):
        ...
        if msg.type == "user-text-message":
            # Extract the text from the message
            user_text = msg.data.get("text", "")
            logger.info(f"Processing text message: {user_text}")
            
            # Create user message in OpenAI format and append to conversation
            user_message = {"role": "user", "content": user_text}
            messages = [user_message]
            
            # Use LLMMessagesAppendFrame to add the message to conversation and run the LLM
            llm_append_frame = LLMMessagesAppendFrame(messages=messages, run_llm=True)
            logger.info(f"Queuing LLMMessagesAppendFrame with run_llm=True for message: {user_text}")
            await task.queue_frames([llm_append_frame])
            logger.info(f"Successfully queued LLMMessagesAppendFrame")
