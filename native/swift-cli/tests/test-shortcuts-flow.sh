...
PERM_RESULT=$($CLI_PATH permissions accessibility)
echo "$PERM_RESULT" | jq -r '.data' | jq '.'
echo ""

# 2. Check current settings
echo "2. Current settings for shortcuts:"
$CLI_PATH settings get | jq -r '.data' | jq '. | {shortcutModifiers, shortcutKeyCode, outputFormat}'
echo ""

# 3. Start shortcuts daemon in background
echo "3. Starting shortcuts daemon in background..."
$CLI_PATH shortcuts --debug > shortcuts.log 2>&1 &
SHORTCUTS_PID=$!
...
$CLI_PATH paste --simulate | jq -r '.data' | head -10
echo ""
